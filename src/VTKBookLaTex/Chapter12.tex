...
vtkPolyDataNormals$\index{vtkPolyDataNormals!application}$ *skinNormals = vtkPolyDataNormals::New();
...
\begin{lstlisting}[language=C++, caption={Improving the visualizarion of the isosurface.}, escapechar=\$]
vtkActor *skin = vtkActor::New();
  skin->SetMapper(skinMapper);
...
vtkStripper$\index{vtkStripper!application}$ *boneStripper = vtkStripper::New();
...
\begin{lstlisting}[language=TCL, caption={Smooth the Triangle Vertices.}, escapechar=\%]
vtkSmoothPolyDataFilter%\index{vtkSmoothPolyDataFilter!application}% smoother
...
\begin{lstlisting}[language=TCL, caption={Generate Triangle Strips.}, escapechar=\%]
vtkStripper%\index{vtkStripper!application}% stripper
...
\begin{lstlisting}[language=TCL, caption={Write the Triangles to a File.}, escapechar=\%]
vtkPolyDataWriter%\index{vtkPolyDataWriter!application}% writer
...
vtkPolyDataReader%\index{vtkPolyDataReader!application}% $reader
  $reader SetFileName $filename
set mapper [eval mkname $name PolyDataMapper]
vtkPolyDataMapper%\index{vtkPolyDataMapper!application}% $mapper
...
proc AddStock {prefix name x y z} {
  ...
  # create labels
  vtkTextSource%\index{vtkTextSource!application}% $prefix.TextSrc
  ...
  vtkPolyDataReader $prefix.PolyDataRead
    $prefix.PolyDataRead SetFileName
		"../../../vtkdata/$prefix.vtk"
  vtkTubeFilter%\index{vtkTubeFilter!application}% $prefix.TubeFilter
    $prefix.TubeFilter
    SetInputConnection \
  ...
  vtkTransform%\index{vtkTransform!application}% $prefix.Transform
	$prefix.Transform Translate 0 0 $zpos
	$prefix.Transform Scale 0.15 1 1
  vtkTransformPolyDataFilter%\index{vtkTransformPolyDataFilter!application}% $prefix.TransformFilter
...
vtkTransform *VTransform = vtkTransform::New();
  vtkTransformPolyDataFilter$\index{vtkTransformPolyDataFilter!application}$ *VTransformFilter =
...
vtkProperty\index{vtkProperty!application} *tomato = vtkProperty::New();
...
\begin{lstlisting}[language=TCL, caption={Display the computational grid.}, escapechar=\%]
# read data
...
vtkStructuredGridGeometryFilter%\index{vtkStructuredGridGeometryFilter!application}% floorComp
  floorComp SetExtent 0 37 0 75 0 0
  floorComp SetInputConnection [pl3d GetOutputPort]
vtkPolyDataMapper%\index{vtkPolyDataMapper!application}% floorMapper
...
# outline
vtkStructuredGridOutlineFilter%%\index{vtkStructuredGridOutlineFilter!application} outline
...
\begin{lstlisting}[language=TCL, caption={Use the computational grid to seed streamlines and then generate streamtubes.}, escapechar=\%]
vtkStructuredGridGeometryFilter%\index{vtkStructuredGridGeometryFilter!application}% seedsComp
...
# create tubes
vtkTubeFilter%\index{vtkTubeFilter!application}% tubes
...
\begin{lstlisting}[language=TCL, caption={Script to generate the blow molding image.}, escapechar=\%]
...
vtkUnstructuredGridReader%\index{vtkUnstructuredGridReader!application}% reader
  reader SetFileName "$VTK_DATA_ROOT/Data/blow.vtk"
  reader SetScalarsName "thickness9"
  reader SetVectorsName "displacement9"
vtkWarpVector%\index{vtkWarpVector!application}% warp
