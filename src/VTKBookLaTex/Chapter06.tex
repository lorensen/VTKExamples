...
\begin{figure}[!htb]
	...
	\begin{subfigure}[h]{0.76\linewidth}
		...
		vtkStructuredPointsReader%\index{vtkStructuredPointsReader!example}% reader
		  reader SetFileName "$env(VTK_TEXTBOOK_DATA)/carotid.vtk"
		vtkThresholdPoints%\index{vtkThresholdPoints!example}% threshold
...
\begin{figure}[!htb]
	...
	\begin{subfigure}[h]{0.76\linewidth}
		\begin{lstlisting}[language=TCL, caption={}, escapechar=\%]
			...
			vtkThresholdPoints%\index{vtkThresholdPoints}% threshold
			  threshold SetInputConnection [reader GetOutputPort]
			  threshold ThresholdByUpper 275
			vtkStreamTracer%\index{vtkStreamTracer!example}% streamers
			...
			vtkTubeFilter%\index{vtkTubeFilter!example}% tubes
