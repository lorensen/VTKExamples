...
\subsection{Warping}
\index{vector algorithms!warping|(}
...
\begin{figure}[htb]
	...
	\end{subfigure}
	\caption{Mapper object design. Graphics mapper shown (e.g., vtkPolyDataMapper\index{vtkPolyDataMapper}) maps polygonal data through graphics library primitives. Writer shown (e.g., vtkSTLWriter) writes polygonal data to stereo lithography format.}\label{fig:Figure6-36}
\end{figure}
\index{filter object!implementation|)}
...
\begin{figure}[!htb]
	...
	\begin{subfigure}[h]{0.96\linewidth}
		...
		vtkQuadric$\index{vtkQuadric!example}$ *quadric = vtkQuadric::New();
		  quadric->SetCoefficients(.5,1,.2,0,.1,0,0,.2,0,0);
		vtkSampleFunction$\index{vtkSampleFunction!example}$ *sample = vtkSampleFunction::New();
		  sample->SetSampleDimensions(50,50,50);
		  sample->SetImplicitFunction(quadric);
		vtkContourFilter$\index{vtkContourFilter}\index{vtkContourFilter!example}$ *contour = vtkContourFilter::New();
		  contour->SetInputConnection(sample->GetOutputPort());
		  contour->GenerateValues(5,0,1.2);
		vtkPolyDataMapper$\index{vtkPolyDataMapper!example}$ *contourMapper = vtkPolyDataMapper::New();
		  contourMapper->SetInputConnection( contour->GetOutputPort());
		  contourMapper->SetScalarRange(0,1.2);
		vtkActor$\index{vtkActor!example}$ *contourActor = vtkActor::New();
		  contourActor->SetMapper(contourMapper);
		// Create outline
		vtkOutlineFilter$\index{vtkOutlineFilter!example}$ *outline = vtkOutlineFilter::New();
...
\begin{figure}[!htb]
	...
	\begin{subfigure}[h]{0.76\linewidth}
		\begin{lstlisting}[language=TCL, caption={}, escapechar=\%]
		vtkStructuredPointsReader reader
		  reader SetFileName "$env(VTK_TEXTBOOK_DATA)/carotid.vtk"
		vtkThresholdPoints threshold
		  threshold SetInputConnection [reader GetOutputPort]
		  threshold ThresholdByUpper 200
		vtkMaskPoints%\index{vtkMaskPoints}\index{vtkMaskPoints!example}% mask
		  mask SetInputConnection [Threshold GetOutputPort]
		  mask SetOnRatio 10
		vtkConeSource%\index{vtkConeSource}\index{vtkConeSource!example}% cone
		  cone SetResolution 3
		  cone SetHeight 1
		  cone SetRadius 0.25
		vtkGlyph3D%\index{vtkGlyph3D!example}% cones
		...
		vtkLookupTable lut
		  ...
		  lut Build
		vtkPolyDataMapper%\index{vtkPolyDataMapper!example}% vecMapper
...
\begin{figure}[!htb]
	...
	\begin{subfigure}[h]{0.76\linewidth}
		\begin{lstlisting}[language=TCL, caption={}, escapechar=\%]
			vtkStructuredPointsReader reader
			  reader SetFileName "$env(VTK_TEXTBOOK_DATA)/carotid.vtk"
			vtkPointSource%\index{vtkPointSource!example}% source
			...
			vtkPolyDataMapper%\index{vtkPolyDataMapper!example}% streamerMapper
...
\item 	 Using the filters vtkStreamLine, vtkMaskPoints\index{vtkMaskPoints!exercise}, and vtkGlyph3D\index{vtkGlyph3D!exercise}, create a visualization consisting of oriented glyphs along a streamline.
