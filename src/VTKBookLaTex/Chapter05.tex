...
\index{image data!representation|)}
...
\index{rectilinear grid!representation|)}
...
\index{polygonal data!representation|)}

\item[vtkUnstructuredGrid.\index{unstructured grid!representation|(}\index{vtkUnstructuredGrid}] The dataset type vtkUnstructuredGrid is the most general in terms of its ability to represent topological and geometric structure. Both points and cells are explicitly represented using derived classes of vtkPoints and vtkCellArray\index{vtkCellArray}. The class vtkUnstructuredGrid is similar to vtkPolyData except that vtkUnstructuredGrid must be capable of representing all cell types, not just the limited graphics types (i.e., vertices, lines, polygons, and triangle strips) of vtkPolyData.
...
\index{unstructured grid!representation|)}
...
\begin{figure}[!htb]
	...
	\begin{subfigure}[h]{0.76\linewidth}
		...
		vtkFloatArray$\index{vtkFloatArray!example}$ *scalars = vtkFloatArray::New();
...
\begin{figure}[!htb]
	...
	\begin{subfigure}[h]{0.56\linewidth}
		\begin{lstlisting}[language=C++, caption={}, escapechar=\$]
		vtkImageData$\index{vtkImageData}$ *vol = vtkImageData::New();
...
\index{unstructured grid!example|(}
