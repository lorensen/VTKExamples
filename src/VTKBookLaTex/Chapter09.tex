...
\begin{lstlisting}[language=TCL, caption={Clipping a plane with a sphere and ellipse.}, escapechar=\%]
vtkPlaneSource%\index{vtkPlaneSource!example}% plane
...
vtkTransform trSphere
  ...
  trSphere Inverse
vtkSphere%\index{vtkSphere!example}% sphere
...
vtkTransform trCylinder
  ...
  trCylinder Inverse
  vtkCylinder%\index{vtkCylinder!example}% cylinder
cylinder SetTransform trCylinder
  cylinder SetRadius .3
vtkImplicitBoolean%\index{vtkImplicitBoolean!example}% boolean
  boolean AddFunction cylinder
  boolean AddFunction sphere
vtkClipPolyData%\index{vtkClipPolyData!example}% clipper
...
\begin{lstlisting}[language=TCL, caption={Generating a swept surface.}, escapechar=\%]
# read data file
vtkPolyDataReader%\index{vtkPolyDataReader!example}% reader
  reader SetFileName "$VTK_DATA_ROOT/Data/vtk.vtk"
# create implicit model of VTK
vtkImplicitModeller%\index{vtkImplicitModeller!example}% imp
...
vtkSweptSurface%\index{vtkSweptSurface!example}% sweptSurfaceFilter
...
vtkContourFilter%\index{vtkContourFilter!example}% iso
...
vtkGaussianSplatter$\index{vtkGaussianSplatter!example}$ *lateSplatter = vtkGaussianSplatter::New();
  lateSplatter->SetInput(dataSet);
  lateSplatter->SetSampleDimensions(50,50,50);
  lateSplatter->SetRadius(0.05);
  lateSplatter->SetScaleFactor(0.005);
vtkContourFilter$\index{vtkContourFilter!example}$ *lateSurface = vtkContourFilter::New();
  lateSurface->SetInput(lateSplatter->GetOutput());
  lateSurface->SetValue(0,0.01);
vtkPolyDataMapper$\index{vtkPolyDataMapper!example}$ *lateMapper = vtkPolyDataMapper::New();
...
\begin{figure}[htb]
	...
	\begin{subfigure}[h]{0.76\linewidth}
        ...
        vtkMCubesReader%\index{vtkMCubesReader!example}% reader
          reader SetFileName \
            "$env(VTK_TEXTBOOK_DATA)/pineRoot/pine_root.tri"
        vtkPolyDataConnectivityFilter%\index{vtkPolyDataConnectivityFilter!example}% connect
          connect SetInputConnection [reader GetOutputPort]
          connect SetExtractionModeToLargestRegion
        vtkPolyDataMapper%\index{vtkPolyDataMapper!example}% isoMapper
        ...
        vtkOutlineFilter%\index{vtkOutlineFilter!example}% outline
...
\begin{figure}[htb]
	...
	\begin{subfigure}[h]{0.76\linewidth}
        ...
        vtkDecimate%\index{vtkDecimate!example}% deci
...
\begin{figure}[htb]
	...
	\begin{subfigure}[h]{0.76\linewidth}
        \begin{lstlisting}[language=TCL,  caption={}, numbers=none, frame=none, escapechar=\%]
        # texture
        vtkStructuredPointsReader%\index{vtkStructuredPointsReader!example}% texReader
        ...
        #
        vtkBYUReader%\index{vtkBYUReader!example}% byu
          byu SetGeometryFileName "$VTK_DATA_ROOT/Data/motor.g"
          byu SetPartNumber 1
        vtkPolyDataNormals normals
          normals SetInputConnection [byu GetOutputPort]
        vtkImplicitTextureCoords%\index{vtkImplicitTextureCoords!example}% tex1
          tex1 SetInputConnection [normals GetOutputPort]
          tex1 SetRFunction planes
        vtkDataSetMapper%\index{vtkDataSetMapper!example}% byuMapper
...
\item Several examples in the text (e.g., deciFran.tcl and deciHawa.tcl) use the class vtkDecimate. Modify these examples to use the topology modifying progressive decimation algorithm (implemented in vtkDecimatePro\index{vtkDecimatePro!exercise}). How much greater reduction can you achieve?
